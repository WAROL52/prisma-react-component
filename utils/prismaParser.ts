...
function linkAllRelation(models: PrismaModel[]) {
  models.forEach(model=>{
    const fields = model.fields.filter(field=>field.fieldType == "relation")
    fields.forEach(field=>{
      const otherModel = models.find(model=>model.modelName == field.originalType)
      if (!otherModel) {
        return
      }
...
function syncRelation(model: PrismaModel,otherModel: PrismaModel,field:PrismaFieldRelation,otherField:PrismaFieldRelation){
  ...
  if (field.type.type.includes("[]")) {
   if (otherField.type.type.includes("[]")) {
    ...
    field.type.relationType="many-to-one"
  }
  else {
    if (otherField.type.type.includes("[]")) {
    ...
    field.type.relationType="one-to-one"
  }
}
...
function getPrismaEnums(data:string):PrismaEnumType {
  ...
  prismaModelRegexResults.map(result=>{
    ...
    //@ts-ignore
    enumType[enumInfo.name]=enumInfo.values
  })
  return enumType
}
...
function getFieldScalar(field:PrismaFieldRegexResult,row:string,rows:string[]):null|PrismaFieldScalar {
  const [type,originalType]=getPrismaFieldType(row,rows) as [PrismaFieldScalar["type"],PrismaFieldScalar["originalType"]]
...
function getFieldRelation(field:PrismaFieldRegexResult,row:string,rows:string[]):null|PrismaFieldRelation {
...
function getPrismaFieldType(row:string,rows:string[]):[PrismaField['type'],PrismaField['originalType']] {
...
},originalType] as [PrismaFieldTypeRelation,PrismaFieldTypeOrinale]
}
